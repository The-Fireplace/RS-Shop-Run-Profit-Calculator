...
public final class Database implements Serializable
{
    ...
    private final HashMap<Integer, HashMap<String, Object>> itemDatabase;
    ...
    public static HashMap<String, ArrayList<Integer>> getAllIdentsForItem() {
        return Objects.requireNonNullElseGet(instance.allIdentsForItem, () -> instance.allIdentsForItem = new HashMap<>());
    }
    ...
    public static void setData(int identifier, EnumDataKey dataKey, Object value) {
        if (!value.getClass().isAssignableFrom(EnumDataKey.getDataType(dataKey))) {
            throw new IllegalArgumentException(value + " is not valid for the data type " + dataKey.name());
        }
    ...
    public static float getDefaultProfitMarginPercent(int identifier) {
        return Float.parseFloat(String.format("%.2f", (1 - ((float) getDefaultInitialPrice(identifier)) / ((float) getGEPrice(identifier))) * 100));
    }
    ...
    public static float getProfitMarginPercent(int identifier, Pair<Integer, Integer> data) {
        return Float.parseFloat(String.format("%.2f", (1 - ((float) getInitialPrice(identifier, data)) / ((float) getGEPrice(identifier))) * 100));
    }
